...
**关键词：** `时间范围` `时间粒度` `3小时` `2026-01-01` `2026-06-01` `timestamp` `152天`
...
**关键词：** `异常` `abnormal` `equipment_status` `350条` `7.2%` `异常率` `R00001` `R04606`
...
- 共 350 条异常记录，占 7.2%
- 四栋建筑均匀分布
- 典型案例：R00001（教学楼凌晨设备状态异常）、R00024（图书馆 COP 偏低且设备状态异常）、R04606（实验楼全库最高电耗 690.04 kWh）
...
**关键词：** `COP` `制冷性能系数` `cooling_load_kwh` `hvac_kwh` `2.99` `2.2` `告警` `能效`
...
- 当前样例数据 COP 均值约 2.99，范围 2.31～3.24
- COP < 2.2 为告警级别，需排查设备；当前样例最低 COP 为 2.31，未触发低 COP 告警
...
**关键词：** `能耗对比` `electricity_kwh` `建筑对比` `实验楼` `BLD-D` `410.5 kWh` `高功率设备`
...
**预期回答要点：**
- BLD-D 实验楼电耗最高，3h 均值 410.5 kWh，最高单次 690.0 kWh
- BLD-B 办公楼最低，均值 235.2 kWh
...
**关键词：** `异常判定` `告警` `阈值` `COP < 2.2` `非工时` `均值+2σ` `equipment_status` `abnormal`
...
- 规则二：同建筑同小时段电耗 > 均值 + 2σ → 电耗偏高告警
- 规则三：COP < 2.2 → 低 COP 告警
- 规则四：00:00-06:00 非工时高负荷作为异常原因标签辅助解释
...
**预期回答要点：**
- R00024（2026-01-01 21:00）：电耗 402.01 kWh，COP=2.34，设备状态为 abnormal
...
- 当前样例数据 COP 均值 2.99，夜间偏低时应结合设备状态、电耗基线和水温差综合判断
...
- 建议告警阈值：electricity > 615 kWh/3h，COP < 2.2
