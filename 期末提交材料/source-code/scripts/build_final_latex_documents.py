...
SRS_ABSTRACT = r"""
本文档为课程期末项目《\projectname》的软件需求规格说明书，采用正式 SRS 结构描述系统目标、业务背景、用户角色、系统边界、功能需求、非功能需求、数据需求、外部接口和验收标准。本文档的需求基线来自项目最终实现版本：系统已形成 FastAPI 后端、Vue 前端、建筑能耗样例数据、异常分析、三维楼层态势、工单闭环、知识库问答、可选外部大模型和 MCP Server。文档中的功能项与代码仓库、测试脚本和最终演示路径保持一致，可作为评审、验收、答辩和后续维护的需求依据。
"""
...
NFR-02 & 可测试性 & 后端应提供 pytest 测试，前端应可构建，项目应提供一键检查脚本。 & \code{check-project.ps1} \\
...
\midrule
\endhead
许奕 & 项目规划、整合、测试、文档、最终封版 & README、任务说明、整合总结、最终文档、测试检查 & 统一接口、统一文档、最终质量控制 \\
...
\subsection{自检控制}
项目提供 \code{scripts/check-project.ps1} 作为最终质量闸门，检查内容包括 Python 语法、后端 pytest 和前端构建。脚本失败即退出，避免出现测试失败但仍显示成功的误判。
...
\begin{enumerate}
    ...
    \item 演示路径覆盖项目亮点，并能应对刷新、筛选、点击、导出和问答。
...
\midrule
\endhead
需求审查 & 功能是否覆盖老师要求和演示路径 & SRS、验收报告 \\
设计审查 & 架构是否清晰、接口是否一致、数据是否可追溯 & SDD、API 文档 \\
代码检查 & 路由、服务、前端页面、脚本是否能运行 & 源码目录、构建结果 \\
自动化测试 & 后端接口、服务、MCP、LLM 封装 & pytest 结果 \\
人工验收 & 浏览器操作、3D 联动、工单持久化、报告生成 & 测试与验收报告 \\
安全检查 & \code{.env}、API Key、运行期数据是否被排除 & \code{.gitignore}、提交清单 \\
\bottomrule
\end{longtable}
...
外部模型 Key 泄露 & 中 & 高 & 真实 Key 仅放本地 \code{.env}，提交前扫描 & 已控制 \\
外部模型不可用 & 中 & 中 & 本地知识库兜底，LLM 可配置关闭 & 已控制 \\
MCP 依赖影响运行 & 低 & 中 & requirements 声明依赖，提供单独启动脚本和测试 & 已控制 \\
PPT/视频未完成 & 中 & 中 & 软件侧完成，最终提交说明列为材料项 & 待补充 \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
源码 & \code{backend/}、\code{frontend/}、\code{scripts/} & 纳入 Git 管理，提交前运行检查 \\
...
最终材料 & \code{期末提交材料/} & 提交前整理源码、PDF、说明和演示材料 \\
\bottomrule
\end{longtable}

\section{变更管理}
项目后期不再进行第三轮任务分配，而采用集中优化方式。原因是系统主链路已经形成，继续分散开发会增加冲突和回归风险。后期变更原则为：只接受能明显提升演示质量、文档质量或验收通过率的变更；每次变更后检查是否影响 README、API、测试和提交材料；涉及密钥、数据和依赖的变更必须额外检查安全性。
...
\section{管理结论}
从软件工程管理角度看，本项目已经完成范围定义、计划执行、多人分工、过程控制、质量验证、风险收敛和最终文档化。系统不需要再次分配开发任务，剩余工作集中在 PPT、视频和最终压缩包提交。项目具备最终提交条件。
"""
...
USER_ABSTRACT = r"""
本文档为《\projectname》的用户手册与部署说明，面向同学、评审人员和后续维护人员，说明如何获取代码、放置环境变量、安装依赖、启动后端、启动前端、启动 MCP Server、使用主要功能、处理常见问题和准备演示。
"""
...
\subsection{决策报告}
决策报告页汇总当前范围内的能耗、异常、工单和优化建议，并提供简单收益模拟。该页面适合答辩时展示系统的管理价值。
...
\midrule
\endhead
\code{data/samples/energy_records.csv} & 2,976 条能耗样例记录 & 查询、统计、异常分析、图表展示 \\
...
"""
    + make_longtable(
        ...
        [
            ...
            ("DQ-08", "密钥不入库", "真实 API Key 只能放在本地 \code{.env}，不得写入源码、文档、PDF 或提交材料。", "提交前执行模式搜索。"),
...
"""
    + make_longtable(
        ...
        [
            ...
            ("异常算法", "规则组合", "规则可解释，适合答辩说明，避免黑箱模型难以验证。", "机器学习模型"),
...
\section{补充经济分析}
\subsection{工作包成本明细}
本节将 240 小时课程项目投入进一步分解到更细工作包，便于说明成本不是简单估算，而是与需求、设计、开发、测试、文档和演示准备相关。
"""
    + make_longtable(
        "工作包成本分解明细",
...
"""
    + make_longtable(
        ...
        [
            ...
            ("R-04", "业务逻辑单薄", "教师可能认为项目只是查询页面。", "增加楼层分析、设备分析、异常解释、工单闭环和报告。"),
            ("R-05", "外部模型不可用", "问答功能演示失败。", "本地知识库优先，外部模型可选。"),
            ("R-06", "真实密钥泄露", "GitHub 公开后造成安全和费用风险。", ".env 不入库，提交前模式搜索。"),
            ("R-07", "文档版面错误", "PDF 图表重叠或表格出页影响评分。", "使用 LaTeX 重写并检查 Overfull 日志和页面预览。"),
            ("R-08", "演示刷新丢工单", "交互逻辑显得不完整。", "工单写入运行期 JSON。"),
            ("R-09", "前端构建失败", "最终源码不可复现。", "提交前运行 Vite build。"),
            ("R-10", "后端测试失败", "接口口径不可信。", "pytest 作为提交前必须检查项。"),
            ("R-11", "PPT 与系统不一致", "答辩展示和实际功能脱节。", "PPT 以最终系统截图和文档为准。"),
            ("R-12", "压缩包结构混乱", "教师难以找到正式文档和源码。", "使用统一提交材料目录。"),
            ("R-13", "运行说明不足", "同学或教师拉取后无法启动。", "README 和用户手册写明完整步骤。"),
...
"""
    + make_longtable(
        ...
        [
            ...
            ("提交包", "源码、文档、PPT、视频", "结构清晰，可直接上传。", "目录清单"),
        ],
    )
)
...
"""
    + make_longtable(
        ...
        [
            ...
            ("运行自检", "项目根目录", "执行 \code{scripts/check-project.ps1}。", "确认测试和构建通过。"),
        ],
    )
    + r"""
\subsection{常见问题排查}
"""
    + make_longtable(
        ...
        [
            ...
            ("提交包太大", "包含 \\code{node_modules} 或 \\code{dist}。", "按提交说明删除依赖和构建产物。", "高"),
        ],
    )
)
...
SRS_REVIEW_TOPICS = [
    ...
    "MCP 工具", "MCP 资源", "接口错误", "空数据状态", "权限边界", "密钥安全", "部署说明", "测试口径", "验收口径", "演示路径",
    "性能边界", "扩展方向", "维护责任", "提交材料", "文档一致性", "风险说明", "配置项", "日志提示", "最终结论", "教师检查点",
]
    ...
    + make_longtable(
        ...
        [
            (
                f"SR-{index:02d}",
                topic,
                f"检查“{topic}”是否已经在需求、界面、接口或验收说明中被明确描述，避免只停留在口头说明。",
    ...
    + make_longtable(
        ...
        [
            (
                f"SD-{index:02d}",
                topic,
                f"确认“{topic}”在设计中有明确位置、输入输出和失败处理，避免模块之间职责模糊。",
                "可在 SDD、代码结构或接口契约中追踪。",
            )
...
SEE_DETAIL_TOPICS = [
    ...
    "MCP 集成投入", "测试投入", "文档投入", "演示准备投入", "人员单价", "工具成本", "部署成本", "模型调用成本", "维护成本", "数据接入成本",
...
SEE_EXPANSION += (
    r"""
\subsection{经济评价检查项}
该表用于把教师要求的成本、收益、定价、融资和评价内容逐项落到项目本身，避免经济分析与系统功能脱节。
"""
    + make_longtable(
        "经济评价检查项",
...
SEM_CONTROL_TOPICS = [
    "任务分解", "成员边界", "第一次整合", "第二次整合", "最终优化", "接口控制", "数据控制", "代码审查", "测试闸门", "文档闸门",
    "安全闸门", "提交闸门", "风险跟踪", "变更记录", "沟通机制", "质量目标", "配置管理", "版本管理", "依赖管理", "运行环境",
    "演示准备", "PPT 协同", "视频协同", "最终打包", "密钥排查", "PDF 检查", "图表检查", "表格检查", "README 检查", "部署检查",
...
SEM_EXPANSION += (
    r"""
\subsection{管理控制项}
以下控制项覆盖项目过程监控和最终交付管理，是从“能做出来”到“能稳定提交”的管理保证。
"""
    + make_longtable(
        "管理控制项",
        ...
        [
            (
                f"MC-{index:02d}",
                topic,
                f"对“{topic}”设置明确检查方式，发现问题后由整合者修复或协调相关同学补齐。",
                "检查清单、提交记录或最终文档。",
            )
    ...
    + make_longtable(
        ...
        [
            (
                f"RT-{index:02d}",
                topic,
                f"执行与“{topic}”相关的页面操作、接口请求或脚本检查，记录是否出现错误。",
                "结果与正式文档说明一致。",
            )
...
USER_TRAINING_TOPICS = [
    ...
    "查看日报", "提出问题", "选择模型", "理解回退", "启动 MCP", "查看 API 文档", "运行测试", "编译文档", "检查密钥", "整理提交包",
    "打开正式 PDF", "准备 PPT", "录制视频", "答辩演示", "故障排查",
]

USER_EXPANSION += (
    r"""
\subsection{培训与演示任务清单}
该清单面向第一次运行项目的同学，用于保证其能够独立完成安装、演示和基本排查。
"""
    + make_longtable(
        "培训与演示任务清单",
        "L{1.8cm}L{3.2cm}L{6.4cm}L{4.0cm}",
...
SRS_DEFENSE_TOPICS = [
    "为什么选择建筑能源管理场景", "为什么需要楼层维度", "为什么异常优先展示", "为什么健康楼层不展示空图", "为什么工单创建后直接处理中",
    "为什么需要持久化工单", "为什么数据浏览不和 3D 联动", "为什么统计分析承担异常查询", "为什么需要 COP 指标", "为什么需要设备状态规则",
    "为什么需要夜间负荷规则", "为什么使用规则而非黑箱模型", "为什么保留 CSV 导出", "为什么需要运营日报", "为什么接入本地知识库",
    "为什么外部模型可选", "为什么 MCP 与 REST 共用服务层", "为什么真实密钥不能入库", "为什么样例数据覆盖多个月", "为什么不提供手工插入数据",
    "如何证明数据不是写死页面", "如何证明异常条数一致", "如何证明楼层联动有效", "如何证明刷新后状态保留", "如何证明模型失败可回退",
    "如何证明接口契约稳定", "如何证明系统可复现运行", "如何证明测试覆盖核心功能", "如何证明文档与系统一致", "如何证明经济分析与项目相关",
    "教师可能检查 README", "教师可能检查 SRS", "教师可能检查 SDD", "教师可能检查 SEE", "教师可能检查 SEM",
    "教师可能要求演示启动", "教师可能要求刷新页面", "教师可能要求查看源码", "教师可能要求查看 API", "教师可能要求查看提交包",
    "后续可接入真实楼宇数据", "后续可增加用户权限", "后续可替换数据库", "后续可增加预测模型", "后续可接入真实工单系统",
]
    ...
    + make_longtable(
        ...
        [
            (
                f"QA-{index:02d}",
                topic,
                f"围绕“{topic}”说明需求来源、业务价值、验收方式和与系统功能的对应关系。",
                "SRS、页面演示或接口返回。",
            )
            for index, topic in enumerate(SRS_DEFENSE_TOPICS, start=1)
        ],
    )
)

SRS_CHANGE_TOPICS = [
    ("C-01", "从单日数据扩展为多月数据", "解决数据量太少、趋势不足的问题，使系统能展示 2026-03-01 至 2026-06-01 的连续记录。"),
    ("C-02", "增加楼层筛选", "把建筑级分析细化到楼层级，支撑 3D 楼层点击和异常定位。"),
    ("C-03", "统计分析承接异常查询", "将异常相关内容集中在统计分析页，数据浏览保持全量查询职责。"),
    ("C-04", "健康楼层简化展示", "健康楼层只显示健康提示，避免空图表降低演示质量。"),
    ...
    ("C-14", "增加提交材料目录", "将源码、正式 PDF、LaTeX 源和演示材料分区整理。"),
    ("C-15", "增加安全检查", "确保提交材料不包含真实 .env 和 API Key。"),
]
...
SDD_AUDIT_TOPICS = [
    ...
    "前端导航策略", "组件拆分策略", "图表加载策略", "3D 状态同步策略", "工单排序策略", "报告生成策略", "文档生成策略", "提交包同步策略",
]
    ...
    + make_longtable(
        ...
        [
            (
                f"DA-{index:02d}",
                topic,
                f"检查“{topic}”从界面入口、接口调用、服务处理到返回展示是否有明确设计，避免只实现单点功能。",
                "SDD 图、模块表或源码路径。",
            )
...
SEE_FINANCE_TOPICS = [
    ...
    "答辩展示价值", "课程学习价值", "试点推广价值", "保守 ROI", "基准 ROI", "乐观 ROI", "回收期", "盈亏平衡点",
]
...
SEM_EVIDENCE_TOPICS = [
    ...
    "安全检查", "构建日志", "测试日志", "正式 PDF", "LaTeX 源文件", "最终提交说明", "演示脚本", "PPT 准备", "视频准备", "提交目录",
...
SEM_EXPANSION += (
    r"""
\subsection{过程证据清单}
过程证据清单用于证明项目并非只提交最终代码，而是具备需求、设计、开发、测试、经济分析、管理控制和交付整理的完整过程。
"""
    + make_longtable(
        "过程证据清单",
        "L{1.7cm}L{4.0cm}L{6.2cm}L{3.6cm}",
        ...
        [
            (
                f"PE-{index:02d}",
                topic,
                f"“{topic}”用于支撑软件工程管理过程，能够说明项目从任务分解到最终提交的演进路径。",
                "仓库文档或提交材料目录。",
            )
