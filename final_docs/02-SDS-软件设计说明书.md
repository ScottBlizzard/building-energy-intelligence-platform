...
> 本文档遵循 **IEEE Std 1016-2009** 软件设计说明书思想编写，并结合课程项目设计文档的通用组织方式安排内容结构。
...
| **文档标题** | 软件设计说明书 (Software Design Specification) |
| **文档版本** | v1.3 |
| **文档状态** | 正式提交版 |
...
| 版本 | 日期 | 修改人 | 变更说明 |
| :--- | :--- | :--- | :--- |
| v1.0 | 2026-04-30 | 项目组 | 形成过程版 SDS，覆盖前后端分离、数据、接口、MCP、测试和部署。 |
| v1.1 | 2026-05-31 | 项目组 | 补充角色登录、工单闭环、时间沙盘、预算、ROI、可信问答和业务闭环设计。 |
| v1.2 | 2026-06-06 | 项目组 | 按 IEEE 1016 设计说明思想重构，补充涉众关注点、设计视图、模块设计、子系统详细设计、数据库、接口、安全、非功能、设计理由和附录。 |
| v1.3 | 2026-06-16 | 项目组 | 结合课程项目文档通用写法补充关键设计决策、模块依赖关系、质量属性分配、端到端数据与控制闭环、设计验证矩阵，并优化服务器部署架构图。 |
...
本软件设计说明书（Software Design Specification, SDS）旨在详细描述“基于大模型的建筑能源智能管理与运维优化系统”的系统架构、模块划分、关键算法、接口契约、数据存储、安全机制、部署方案和设计取舍。本文档是将需求用例、业务规则和非功能需求转化为工程实现方案的设计蓝图。
...
![SDS-2.3-系统上下文图](images/SDS-2.3-系统上下文图.png)
...
- **设计要点**：采用单页工作台方式，避免课程演示中多路由跳转带来的状态丢失；所有 REST 调用集中在 `api.js`，统一处理 Bearer token、`operator_id` 兼容参数和错误。
...
- **数据字典**：`data/dictionaries/energy_records_dictionary.csv`，由数据元信息接口和 MCP 资源读取，输出字段定义与覆盖率。
...
![物理部署架构](images/SDS-3.4-物理部署架构.png)
...
本项目最终演示版采用轻量云服务器部署，前端静态资源由 Nginx 直接托管，REST API 与 MCP streamable-http 由 Nginx 反向代理到本机后端进程。后端服务与 MCP 服务均由 systemd 托管，避免课堂演示时依赖本地开发终端。

![服务器部署架构图](images/SDS-3.5-server-deployment-architecture.svg)

服务器部署职责如下：

| 部署元素 | 运行位置 | 职责 |
| :--- | :--- | :--- |
| Nginx | 服务器公网 80 端口 | 托管 `frontend/dist`，并将 `/api/` 转发至 FastAPI，将 `/mcp` 转发至 MCP 服务。 |
| Vue 前端静态资源 | `/opt/thymic_project/app/frontend/dist` | 提供登录、总览、数据浏览、工单、预算、ROI、决策报告和智能问答页面。 |
| FastAPI 后端 | `127.0.0.1:8000` | 提供 `/api/v1` REST 接口，处理认证、数据、分析、工单、预算、ROI、决策和演示重置。 |
| MCP Server | `127.0.0.1:8765` | 以 streamable-http 暴露 MCP Tools/Resources，复用后端服务层和可信问答校验逻辑。 |
| 文件数据层 | `data/samples`、`data/runtime`、`knowledge_base` | 保存样例能耗数据、运行期工单/预算/沙盘状态和本地知识库。 |
| 可选增强 | MySQL、外部 LLM | 配置 `DATABASE_URL` 或模型 API Key 后启用；未配置时系统保持可运行。 |

该部署结构保证教师或评审人员可以从任意电脑访问公网地址完成演示，同时保留本地开发部署方式和默认文件模式，降低部署复杂度。
...
本节记录系统级设计决策，说明为什么当前实现选择轻量、可解释、可演示的架构，而不是引入更重的生产级平台组件。

| 决策编号 | 设计问题 | 备选方案 | 最终决策 | 选择理由与约束 |
| :--- | :--- | :--- | :--- | :--- |
| DD-01 | 课程交付环境中的数据持久化 | 仅 MySQL；仅 CSV/JSON；默认文件模式并保留 MySQL 可选增强 | 默认 CSV/JSON，配置 `DATABASE_URL` 后启用 MySQL | 默认文件模式降低部署门槛，保证教师电脑或轻量云服务器可直接运行；MySQL 设计仍保留数据库建模、事务和后续扩展路径。 |
| DD-02 | Web、AI 助手和 MCP 的业务口径 | 三套独立逻辑；共用部分工具函数；完全复用服务层 | REST API、AI 助手和 MCP Tools 复用 `backend/app/services/` | 避免总览、预算、ROI、报告和智能问答出现不同计算口径；测试也能直接覆盖多入口复用逻辑。 |
| DD-03 | 时间沙盘实现方式 | 修改原始数据；独立模拟数据库；业务日期过滤与干预记录 | 采用沙盘状态文件、业务日期窗口和设备级维修干预 | 不破坏原始样例数据，便于一键重置；工单关闭后的维修影响可以稳定反映到未来异常、预算和报告中。 |
| DD-04 | 异常到工单的闭环粒度 | 单条异常级工单；设备级工单；建筑级批量工单 | 以设备为最小闭环单位 | 同一设备多个异常通常对应同一次现场处理，设备级闭环能减少重复派单，并支撑“关闭后影响未来”的业务解释。 |
| DD-05 | 预算状态与 KPI 评分口径 | 单一百分比；黑盒综合评分；可解释分项评分 | 采用预算执行率、风险带和分项 KPI 评分 | 预算内、预警和超预算有明确边界；KPI 由预算执行、异常改善、工单闭环、ROI 和数据质量等分项构成，便于答辩解释。 |
| DD-06 | ROI 与决策报告 | 只展示节能金额；只输出文字报告；结构化 ROI + 报告摘要 | ROI 分析、方案对比和运营报告共用经济口径 | 节电量、节费、碳减排、回本期和处置建议在预算、ROI、报告中保持一致，降低评审时的数字歧义。 |
| DD-07 | 外部大模型依赖 | 强制外部 LLM；完全本地规则；外部 LLM 可选 + 本地回退 | 默认本地可信回答，外部 LLM 作为增强能力 | 演示不依赖 API Key 和公网稳定性；配置模型后可增强表达，但事实、引用和业务上下文仍由本地服务约束。 |
| DD-08 | 部署形态 | 本地开发服务；GPU 算力服务器；轻量云服务器生产式部署 | 轻量云服务器 + Nginx + systemd，无需 GPU | 项目是数据分析与业务系统，不需要显卡；轻量云服务器更接近教师从公网访问的真实部署方式。 |
| DD-09 | 权限与安全边界 | 无权限；完整生产 IAM；演示 RBAC + 生产 Bearer 约束 | 管理员/工人角色权限在服务层校验，生产部署可启用 Bearer token | 保留关键角色边界和接口校验，不引入与课程目标无关的复杂账号体系；真实密钥仍通过 `.env` 隔离。 |

### 3.7 模块依赖关系

模块依赖以单向服务调用为主，前端、REST、MCP 和 AI 助手不直接读取底层文件，而是通过服务层访问数据和业务对象。

| 上游模块 | 下游模块 | 依赖内容 | 设计约束 |
| :--- | :--- | :--- | :--- |
| 前端工作台 | REST API 适配层 | 登录态、筛选条件、工单操作、预算和 ROI 请求 | 前端只处理展示状态，不复制核心业务计算公式。 |
| REST API 路由 | 业务服务层 | 参数校验、异常映射、响应包装 | 路由层不直接读写 CSV/JSON/MySQL，避免接口逻辑和持久化逻辑耦合。 |
| 数据加载服务 | 分析、预算、ROI、报告服务 | 清洗后的能耗记录、建筑与设备维度、数据元信息 | 所有统计服务必须使用同一过滤结果，保证沙盘日期和建筑筛选一致。 |
| 异常诊断服务 | 工单服务、报告服务、AI 助手 | 异常列表、风险评分、异常解释 | 异常记录由当前可见数据动态计算，工单只持久化人工处置闭环。 |
| 工单状态机 | 权限服务、沙盘服务、预算服务、报告服务 | 派单、接单、提交、复核、关闭和干预登记 | 非法状态跳转必须被拒绝；关闭或忽略动作必须写入 timeline。 |
| 沙盘服务 | 数据加载、异常诊断、预算、决策报告 | 业务日期、未来窗口、反事实结果、设备干预 | 沙盘推进不得改写原始 CSV，只通过状态和干预影响可见结果。 |
| 预算与 ROI 服务 | 决策报告、AI 助手、MCP Tools | 预算执行率、KPI 分项、ROI 指标、方案对比 | 已发生月份使用实际数据，未来或当前窗口使用预测口径，并在返回结果中标识。 |
| 知识库检索服务 | AI 助手、MCP Resources | Markdown 条目、引用片段、术语解释 | 回答必须携带可追溯引用或明确返回本地回退状态。 |
| MCP Server | 业务服务层、知识库服务 | 数据查询、分析、工单上下文、知识问答工具 | MCP Tools 不绕过权限和事实校验逻辑，不暴露任意文件读写能力。 |
| 部署脚本与 systemd | FastAPI、MCP、Nginx | 进程启动、反向代理、健康检查 | 服务器只部署系统运行件，不上传无关过程文档和参考材料。 |

### 3.8 设计质量属性分配

| 质量属性 | 设计支撑 | 验证方式 |
| :--- | :--- | :--- |
| 性能 | Pandas 内存聚合、前端按需筛选渲染、MCP 查询 limit、Nginx 托管静态资源 | 执行后端测试、前端构建和核心接口访问；在轻量 CPU 服务器上验证主要页面可打开。 |
| 可靠性 | 默认 CSV/JSON 可运行、外部 LLM 失败回退、工单状态机拒绝非法跳转、一键演示重置、systemd 托管进程 | 模拟无 MySQL、无 LLM、重复派单、非法工单状态和重置流程，检查接口返回是否稳定。 |
| 安全性 | `.env` 保存真实密钥、`.env.example` 仅保留占位、管理员/工人权限服务校验、生产 Bearer token、Nginx 只暴露必要入口 | 检查仓库无真实密钥；验证工人不能复核他人工单，未授权生产请求被拒绝。 |
| 可维护性 | 路由层、服务层、仓储层、知识库和前端组件分离；REST、MCP 和 AI 助手复用服务层 | 通过代码结构审查和测试覆盖确认新增入口不需要复制核心公式。 |
| 可观测性 | 健康检查接口、工单 timeline、预算/KPI 分项解释、ROI 输入输出摘要、AI 引用和 grounding 状态 | 检查页面和接口能追溯关键业务数字来源，工单闭环动作有时间线记录。 |
| 可部署性 | 轻量云服务器、Nginx、systemd、前端静态构建、后端虚拟环境、MCP HTTP 服务 | 通过服务器公网入口、`systemctl` 状态和健康检查验证部署结果。 |
| 兼容性 | Windows 本地脚本与 Linux 服务器部署并存；默认文件模式与可选 MySQL 并存；本地问答与外部 LLM 并存 | 分别检查本地开发启动、服务器部署启动和无外部增强配置的运行结果。 |
| 可验收性 | SRS-SDS-SEE 追踪矩阵、图示清单、验收脚本、123 项测试基线 | 运行 `scripts/check-project.ps1` 或等价命令，并将关键设计元素映射到测试与需求条目。 |

### 3.9 端到端数据流与控制流

端到端闭环从能耗数据进入系统开始，经过统计分析、异常诊断、工单处置、维修干预、预算 KPI、ROI 和运营报告，最终由前端、AI 助手与 MCP 工具共同呈现。系统不把这些能力设计成孤立模块，而是通过共享服务层保持业务数字和解释口径一致。

![端到端数据与控制闭环](images/SDS-3.9-end-to-end-data-control-flow.svg)

核心流程如下：

1. 数据加载服务读取 `energy_records.csv`、数据字典和运行期状态，按建筑、月份、角色和沙盘日期生成当前可见数据集。
2. 统计分析服务基于当前可见数据计算能耗趋势、异常事件、风险评分、经济损失和碳排影响。
3. 管理员将异常转化为工单，工单状态机负责派发、接单、提交、复核、关闭、驳回和忽略等状态迁移。
4. 工单关闭后登记设备级维修干预，沙盘服务据此影响后续日期窗口中的异常数量、能耗预测和预算改善。
5. 预算、KPI 与 ROI 服务使用同一经济口径计算预算状态、节能收益、节费、碳减排、回本期和方案对比。
6. 决策报告汇总数据、异常、工单、预算、ROI 和沙盘反馈，形成面向运营管理的闭环说明。
7. AI 助手和 MCP Tools 只调用同一业务服务层和知识库检索服务，不维护独立公式，避免智能入口与 Web 页面数字不一致。
...
| 认证权限 | `auth_service.py`、`permission_service.py`、`api/deps.py`、`routes/auth.py` | 演示用户、Bearer token、角色校验、工单归属校验。 |
...
4. 前端 API 封装保存 token，并在后续受保护请求中发送 `Authorization: Bearer demo-token:<user_id>`。
5. 后端通过 `api/deps.py` 解析 Bearer token；生产环境缺失或无效 token 返回 401，token 用户与 `operator_id` 不一致返回 403。
6. 管理员类接口调用 `require_admin_user`；工人类接口调用 `require_worker_user`。开发环境保留 `operator_id` 兼容本地测试和课堂演示。
...
- 工人只能接单和提交本人负责的工单。
- 工人忙碌时不能被重复分配新活动工单。
- 预算、ROI、决策、沙盘写入和演示重置等受保护接口在生产环境要求有效管理员 token。
...
![工单状态转换图](images/SDS-4.3.3-工单转换状态图.png)
...
| 维度 | 权重 | 评分逻辑 |
| :--- | :--- | :--- |
| 预算控制 | 40 | 按月度预算执行率扣分，预算内计满分，超预算越多扣分越多。 |
| 运行效率 | 20 | 根据 COP 水平、暖通能效和单位面积能耗评价运行效率。 |
| 异常风险 | 20 | 根据异常数量、风险分和异常损失评价风险暴露。 |
| 工单闭环 | 15 | 根据工单关闭率、响应及时性和复核闭环情况计分。 |
| 改善贡献 | 5 | 根据已关闭工单带来的预计节省和改善摘要计分。 |

预算分析必须输出超预算楼栋、风险等级、预测执行率、KPI 分项评分和闭环改善摘要。预算状态按执行率分为：`<=100%` 为健康，`100%-103%` 为预警，`>103%` 为超预算。已完整发生的历史月份使用实际值作为月末口径；只有当前未完成月份才进行月末预测。已关闭工单的预计节省进入预算改善汇总，并与异常损失、运营报告和问答使用同一电价、碳排和节省比例口径。
...
![数据分析处理流程图](images/SDS-5.1.2-数据分析处理流程图.png)

#### 5.1.3 核心类/函数设计

![数据与分析核心类图](images/SDS-5.1.3-数据与分析核心类图.png)
...
| `data_loader.get_dataset_meta()` | 生成数据集元信息。 | 数据集和数据字典可读取。 | 返回记录数、字段列表、字段定义、数据字典、字典覆盖率、时间范围和建筑数量。 |
...
![工单核心类图](images/SDS-5.2.2-工单核心类图.png)
...
| `PermissionService` | 服务模块 | `require_admin_user()`、`require_worker_user()`、`build_worker_support()` | 校验管理员/工人权限，并构造工人作业支持信息。 |
...
![工单派单处理复核时序图](images/SDS-5.2.3-工单派单处理复核时序图.png)

**自动待确认队列**

![自动待确认队列时序图](images/SDS-5.2.3-自动待确认队列时序图.png)
...
![关闭工单影响未来时序图](images/SDS-5.3.2-关闭工单影响未来时序图.png)

**资源约束派单**

![资源约束派单时序图](images/SDS-5.3.2-资源约束派单时序图.png)

#### 5.3.4 核心类/函数设计

![时间沙盘与决策核心类图](images/SDS-5.3.4-时间沙盘与决策核心类图.png)
...
| `BudgetKPI` | DTO | `average_score`、`grade`、`score_breakdown`、`score_reasons`、`monthly_details`、`strengths`、`improvements` | 年度预算和运营绩效评分。 |
...
![预算生成闭环改善与ROI候选时序图](images/SDS-5.4.3-预算生成闭环改善与ROI候选时序图.png)

#### 5.4.4 核心类/函数设计

![预算ROI核心类图](images/SDS-5.4.4-预算ROI核心类图.png)
...
| `budget_service.build_budget_kpi()` | 计算年度 KPI 分数和等级。 | 建筑和年份合法；相关月份有可见数据。 | 返回五项分解评分、原因说明、等级、优势和改进建议。 |
...
![智能问答服务时序图](images/SDS-5.5.3-智能问答服务时序图.png)

#### 5.5.4 核心类/函数设计

![智能服务类图](images/SDS-5.5.2-核心服务子系统类图.png)
...
| 状态域 | 主要变量/对象 | 职责 |
| :--- | :--- | :--- |
| 认证状态 | `currentUser`、`authToken`、`setApiOperator()`、`setApiAuthToken()` | 保存当前用户、token 和操作者 ID，并同步 API 请求头。 |
...
![前端展示核心组件类图](images/SDS-5.6.3-前端展示核心组件类图.png)
...
前端通过 `frontend/src/lib/api.js` 统一封装所有 HTTP 调用，页面组件不得直接拼接后端 URL。`request()` 负责附加 `Authorization: Bearer ...`、组合开发兼容的 `operator_id`、设置 JSON 头并解析错误响应；`downloadCsvExport()` 单独处理 Blob 下载。
...
![数据库ER图](images/SDS-6.2-数据库ER图.png)
...
| 数据 | `GET /overview`、`/dataset-meta`、`/buildings`、`/records` | 总览、字段元信息、数据字典、字典覆盖率、建筑清单和记录查询。 |
...
| :--- | :--- | :--- | :--- | :--- |
| `loginUser` / `POST /auth/login` | 用户登录并获取演示 token。 | `auth_service.authenticate_user`、`api/deps.py` | 用户存在、启用且密码匹配。 | 返回公开用户信息和 `demo-token:<user_id>`；前端后续以 Bearer token 调用受保护接口；不返回密码。 |
...
| `acceptPersistentWorkOrder` / `PATCH /work-orders/{id}/accept` | 工人接单。 | `work_order_store.accept_work_order`、`permission_service.require_worker_user` | 调用者为 worker；工单存在；工单状态为 `assigned`；工单指派给当前工人。 | 工单状态变为 `in_progress`；Timeline 增加接单记录；该工人进入忙碌状态。 |
...
| Tool | 用途 |
| :--- | :--- |
| `get_dataset_meta` | 获取字段、字段定义、数据字典、字典覆盖率、建筑、记录数和时间范围。 |
...
| :--- | :--- | :--- |
| `energy://dataset/meta` | `application/json` | 数据集元信息、字段定义、数据字典和字典覆盖率。 |
...
- 前端在调用受保护接口时发送 `Authorization: Bearer demo-token:<user_id>`。
- 生产环境以 Bearer token 作为身份依据；缺失或无效 token 返回 401，token 用户与 `operator_id` 不一致返回 403。
- 开发环境保留 `operator_id` 作为测试和本地演示兼容参数，但前端仍会同步 token。
...
| 创建/派单/复核/忽略工单 | 管理员 | `require_admin_user` 校验，生产环境要求管理员 Bearer token。 |
| 接单/提交工单 | 工人 | `require_worker_user` 校验，并校验 assignee。 |
| 预算、ROI、决策 | 管理员 | 路由层通过 `api/deps.py` 校验管理员 token。 |
| 沙盘写入和演示重置 | 管理员 | 启动、推进、重置和演示重置在生产环境要求管理员 token。 |
| 工人看板 | 指定工人或管理员 | 工人只能读取本人任务，管理员可查看调度视角。 |
...
本章提供需求追踪矩阵，证明 SRS 中的主要用例和需求均被 SDS 中的设计视图、模块、接口和数据结构覆盖，同时避免设计文档创造无法追溯到需求的核心功能。矩阵以 `final_docs/01-SRS-软件需求规格说明书.md` 的用例编号、功能需求和非功能需求为追踪源。

### 11.1 用例与功能需求追踪矩阵

| SRS 需求编号 / 用例 | 需求名称 | 业务规则覆盖 | 对应 SDS 设计视图/章节 | 对应核心组件/API/数据表 |
| :--- | :--- | :--- | :--- | :--- |
| UC-00 / FR-003 | 用户登录与权限管理 | BR-100、BR-101、BR-102、BR-103、BR-104、BR-105 | 4.2.1 用户认证与权限管理；8.1 身份认证；8.2 访问控制 | `auth_service.py`、`permission_service.py`、`api/deps.py`、`routes/auth.py`、`POST /auth/login`、`GET /auth/me` |
| UC-01 / FR-001 | 管理样例数据与演示状态 | BR-110、BR-111、BR-112、BR-113、BR-114 | 3.3.4 数据存储；4.2.3 数据仓储切换；3.5 部署方案 | `data_loader.py`、`demo_service.py`、`POST /demo/reset`、`energy_readings`、CSV/JSON 文件、数据字典 |
...
| UC-08 / FR-005 | 预算执行与闭环改善 | BR-180、BR-181、BR-182、BR-183、BR-184、BR-185 | 4.3.5 预算、KPI 与 ROI；5.4 预算与 ROI 子系统；6.3.3 预算表 | `budget_service.py`、`decision_service.summarize_budget_impact_from_closures()`、`/budget/budgets*`、`budgets` |
...
| FR-001 | 能源数据管理与查询 | BR-110、BR-111、BR-112、BR-113、BR-114、BR-120、BR-121、BR-122、BR-123 | 4.3.1；6.3.1；6.4 | `energy_readings`、`data/samples/energy_records.csv`、数据字典、`GET /records`、`GET /export/csv` |
...
| FR-005 | 预算、ROI 与运营报告 | BR-180、BR-181、BR-182、BR-183、BR-184、BR-185、BR-190、BR-191、BR-192、BR-193 | 4.3.5；5.4；附录D | `budget_service.py`、`roi_service.py`、`decision_service.py`、`budgets` |
...
| :--- | :--- | :--- |
| BR-100、BR-101、BR-102、BR-103、BR-104、BR-105 | 4.2.1 用户认证与权限管理；8.1 身份认证；8.2 访问控制；7.3.1 接口契约 | `auth_service.py`、`permission_service.py`、`api/deps.py`、`POST /auth/login`、`GET /auth/users` |
| BR-110、BR-111、BR-112、BR-113、BR-114 | 3.3.4 数据存储；4.2.3 数据仓储切换；6.4 文件存储模型 | `data/samples/energy_records.csv`、`data/dictionaries/`、`data/runtime/`、`repository.py`、`DATABASE_URL` |
...
| BR-150、BR-151、BR-152、BR-153 | 4.3.3 关键约束；5.2.1 功能描述；7.3.1 接口契约 | `require_worker_user()`、`submit_work_order()`、附件元数据、管理员复核页 |
...
| BR-180、BR-181、BR-182、BR-183、BR-184、BR-185 | 4.3.5 预算、KPI 与 ROI；5.4 预算与 ROI 子系统 | `budget_service.py`、预算风险等级、KPI 分项评分、闭环改善摘要 |
...
| SRS 接口编号 | 接口需求名称 | 对应 SDS 章节 | 对应核心组件/API |
| :--- | :--- | :--- | :--- |
...
| SRS 非功能需求 | 需求名称 | 对应 SDS 章节 | 对应设计机制 |
| :--- | :--- | :--- | :--- |
| NFR-001、NFR-002、NFR-003、NFR-004 | 性能需求 | 9.1 性能设计 | Pandas 内存分析、缓存、MCP limit 参数、前端按筛选渲染。 |
| NFR-010、NFR-011、NFR-012、NFR-013、NFR-014 | 安全性需求 | 8. 安全设计；7.2.2 LLM 接口约束 | `.env` 隔离、`.env.example` 占位、API Key 不返回、生产 Bearer token、管理员/工人权限校验、LLM 降级。 |
...
本矩阵从设计对象角度说明如何验证 SDS 中的关键设计已经落地。SEE 文档中的测试用例负责给出具体执行结果；本节用于建立“设计对象 - 验证输入 - 期望输出 - 通过标准”的复核路径。

| 验证编号 | 设计对象 | 验证输入 | 期望输出 | 通过标准 |
| :--- | :--- | :--- | :--- | :--- |
| DV-01 | 数据加载与元信息 | `energy_records.csv`、数据字典、建筑筛选、月份筛选 | 返回记录列表、字段覆盖率、建筑/设备维度和当前可见数据范围 | 页面、REST API 和 MCP 读取的数据范围一致，缺失字段不导致服务崩溃。 |
| DV-02 | 异常诊断与风险评分 | 含异常能耗、温度、COP 或运行状态的样例记录 | 生成异常列表、异常原因、风险分和经济/碳排影响 | 异常解释与规则公式一致，同一筛选条件下总览、分析页和报告数字一致。 |
| DV-03 | 工单状态机 | 派单、接单、提交、复核、驳回、忽略和重复派单请求 | 合法请求进入下一状态，非法请求返回错误 | 工单 timeline 完整记录操作；工人不能处理非本人任务；已闭环设备不重复派单。 |
| DV-04 | 维修干预与沙盘联动 | 关闭工单、推进沙盘日期、查询未来异常 | 设备级干预影响后续窗口的异常、节电量和报告摘要 | 关闭前后对比可观察到未来异常和经济指标变化，且原始 CSV 不被改写。 |
| DV-05 | 预算状态判定 | 月份、建筑、预算值、实际/预测用电量 | 返回健康、预警或超预算状态以及 KPI 分项 | 执行率不超过 100% 为健康，100% 至 103% 为预警，超过 103% 为超预算；历史月和当前预测月口径有标识。 |
| DV-06 | KPI 综合评分 | 预算执行、异常改善、工单闭环、ROI、数据质量等分项 | 返回总分、分项分和解释文本 | 总分由可解释分项计算，不出现无依据的全月满分；页面和接口显示一致。 |
| DV-07 | ROI 与方案对比 | 建筑、设备、改造措施、投资成本、节能收益 | 返回节电量、节费、碳减排、回本期和方案排序 | ROI 指标单位统一，预算页、ROI 页和运营报告采用同一经济口径。 |
| DV-08 | 决策报告 | 当前沙盘日期、工单闭环状态、预算和 ROI 数据 | 返回未来 7 天预测、异常摘要、预算风险和行动建议 | 报告中的关键数字可追溯到分析、预算、ROI 和工单服务。 |
| DV-09 | AI 可信问答 | 业务问题、知识库条目、可选外部 LLM 配置 | 返回本地或外部模型回答、引用和 grounding 状态 | 无外部 LLM 时仍能回答；外部模型失败时回退本地回答；引用不为空或明确说明无法引用。 |
| DV-10 | MCP 工具服务 | MCP 客户端调用数据、分析、工单、知识库工具 | 返回结构化 JSON 或资源内容 | MCP 结果与 REST/API 页面口径一致，不暴露任意文件读写能力。 |
| DV-11 | 服务器部署 | Nginx、systemd、FastAPI、MCP、前端静态文件 | 公网首页、REST 健康检查和 MCP HTTP 服务可访问 | 教师电脑可访问公网入口；后端和 MCP 在本机端口由 systemd 托管。 |
| DV-12 | 演示重置与可复现 | 一键重置、重新播种运行期状态、重新打开页面 | 系统恢复到预期演示基线 | 重置后工单、预算、沙盘和报告状态可重复，适合课堂演示和答辩复核。 |
...
同“文档修订历史”章节。本附录保留用于统一正式设计文档的附录组织方式。
...
| 服务器部署架构图 | 3.5.3 | Deployment |
| 端到端数据与控制闭环图 | 3.9 | Data and Control Flow |
...
本文档使用图片文件组织系统上下文图、逻辑架构图、部署图、端到端流程图和模块划分图。第 5 章已将主要 UML 类图和交互时序图导出为 PNG，并保留 draw.io 源文件；第 3 章新增服务器部署架构图和端到端闭环图使用 SVG 维护，便于后续直接调整文字和连线。若后续转换为正式 PDF，可直接引用 PNG/SVG，或使用 `.drawio` 源文件重新导出高清图。
...
| 预算预警阈值 | `100%-103%` | 预算状态。 |
| 预算超预算阈值 | `>103%` | 预算状态。 |
| KPI 五项权重 | 40/20/20/15/5 | 预算控制、运行效率、异常风险、工单闭环、改善贡献。 |
| 社会折现率 | 8% | ROI 主判据。 |
...
> **符合标准**：本文档依据 IEEE Std 1016-2009 (IEEE Standard for Information Technology--Systems Design--Software Design Specifications) 的设计说明思想编写，并与本项目 SRS、API 契约、代码结构和最终交付材料保持一致。
