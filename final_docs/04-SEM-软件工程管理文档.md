...
6. 最终演示形态支持本地运行和轻量云服务器部署，服务器侧采用 Nginx 托管前端、systemd 托管 FastAPI 与 MCP HTTP 服务，生产环境受保护接口通过 Bearer token 校验角色。
...
7. **交付材料一致**：保证 SRS、SDS、SEE、SEM、测试验收、用户手册、演示脚本和 README 对项目范围、数据规模、接口、账号和测试结论表述一致。
...
| 管理领域 | 主责人 | 执行机制 | 主要产出 |
|---|---|---|---|
| 范围管理 | 许奕 | 对照课程题目、SRS、SDS 与最终报告事实底稿判断新增内容是否纳入本期交付 | 范围说明、不纳入范围、变更记录 |
...
| 文档与交付 | 许奕 | 按 SRS/SDS/SEE/SEM、验收报告、用户手册、演示指南和提交清单统一检查 | 正式文档包、PPT/演示说明、提交材料 |
...
5. 最终交付不仅要求代码，还要求 SRS、SDS、SEE、SEM、测试验收、用户手册、演示脚本和提交说明相互一致。

因此，本项目的过程模型不是省略过程控制，而是采用“阶段化增量 + 轻量 Scrum”的混合过程：阶段门保证课程交付件完整，Sprint 迭代保证系统持续可运行并能吸收业务口径变更。

![阶段化增量与轻量敏捷过程模型](images/SEM-3.1-agile-incremental-process.svg)
...
| P6 数据口径与工程封版 | 2026-06-11 至 2026-06-12 | 完成 L0-L3 数据质量清洗、MySQL 可切换持久化、资源约束派单、设备级修复和部署权限收敛 | 数据重构、MySQL 后端、123 个 pytest 用例、正式 SRS/SDS/SEM 文档 |
...
| G4 MCP 与文档门 | MCP Tools、Resources、SRS/SDS/SEE/SEM 是否补齐 | MCP 测试通过，正式文档能支撑课程交付 | 补充接口说明、测试和最终提交指南 |
...
| P1 | 正式交付文档 | 满足课程提交要求 | SRS、SDS、SEE、SEM、验收报告 |
...
| S4 最终基础交付 | 2026-05-30 至 2026-06-01 | 形成课程可提交基础版 | MCP、LLM 配置、最终文档、验收报告、提交指南 | MCP Server、SRS/SDS/SEE/SEM 过程版、验收报告 |
| S5 业务闭环升级 | 2026-06-10 至 2026-06-11 | 从看板升级为运营闭环系统 | 角色登录、工单闭环、时间沙盘、预算、ROI、接地问答 | 业务闭环文档、演示脚本、测试增强 |
| S6 质量封版 | 2026-06-11 至 2026-06-12 | 统一数据口径、修复业务约束、补齐正式报告 | L0-L3 数据清洗、MySQL、资源约束派单、设备级修复、正式 SRS/SDS/SEM | 全量测试、正式文档、封版提交 |
...
12. 完成自动化测试、启动脚本、验证脚本、用户手册、演示脚本、验收报告和正式 SRS/SDS/SEE/SEM 文档。
...
1. 新增功能必须能对应到 SRS 用例、SDS 设计章节、业务闭环目标或课程提交要求。
...
| 设计基线 | 《02-SDS-软件设计说明书.md》中的架构、模块、数据库、接口、安全和非功能设计 | 设计调整不得破坏已验收需求 |
...
| WBS 编号 | 层级 | 工作包 | 主要交付物 |
|---|---|---|---|
| 1.0 | 一级 | 项目文档与范围基线 | 《01-SRS-软件需求规格说明书.md》、《02-SDS-软件设计说明书.md》、《03-SEE-软件经济分析与评价.md》、本 SEM、范围基线 |
| 1.1 | 二级 | 需求与范围文档 | 《01-SRS-软件需求规格说明书.md》、用例清单、范围纳入/不纳入说明、验收边界 |
| 1.2 | 二级 | 设计与接口文档 | 《02-SDS-软件设计说明书.md》、《06-api-contract.md》、架构设计、模块设计、接口契约 |
| 1.3 | 二级 | 管理与经济评估材料 | 本 SEM、《03-SEE-软件经济分析与评价.md》、《实验二：软件规模度量报告.md》、《实验三：软件成本估算报告.md》 |
| 1.4 | 二级 | 演示与提交说明 | 《README.md》、用户手册、演示脚本、提交清单、答辩材料说明 |
| 2.0 | 一级 | 工程基础与运行环境 | FastAPI、Vue 3、Vite、依赖配置、本地启动脚本、轻量云部署配置、可选 MySQL 运行配置 |
...
| 2.4 | 二级 | 本地运行与部署环境 | 启动脚本、.env.example、LOCAL_DEV/README 说明、Nginx/systemd 部署配置、前后端联调配置 |
...
| 11.4 | 二级 | 正式报告、实验报告与答辩材料 | SRS、SDS、SEE、SEM、验收报告、实验二/实验三、PPT/Demo 说明 |

### 5.1 WBS 交付物与验收责任

| WBS 工作包 | 交付物 | 验收责任 | 验收依据 |
|---|---|---|---|
| 1.0 项目文档与范围基线 | SRS、SDS、SEE、SEM、规模与成本估算报告、范围基线 | 许奕 | 《01-SRS-软件需求规格说明书.md》、《02-SDS-软件设计说明书.md》、《03-SEE-软件经济分析与评价.md》、本 SEM、《实验二：软件规模度量报告.md》、《实验三：软件成本估算报告.md》 |
| 2.0 工程基础与运行环境 | FastAPI 后端、Vue 前端、依赖配置、启动说明、本地运行和轻量云部署配置 | 王天一、周由 | backend、frontend、deploy/systemd、《README.md》、启动脚本、依赖清单 |
...
|---|---|---|---|---|
| WBS 1.0 项目文档与范围基线 | 第 1 周至第 7 周 | SRS、SDS、SEE、SEM 与范围基线同步 | 文档不能支撑验收和答辩追问 | 第 8 周用统一事实底稿集中封版，保证正式文档与系统一致 |
...
|---|---:|---|---|
| WBS 1.0 项目文档与范围基线 | +1 周 | 第 8 周业务闭环、ROI、MySQL 和数据质量修复改变了正式文档事实口径 | 采用统一事实底稿，集中更新 SRS、SDS、SEE、SEM 和实验二/三引用 |
...
为体现计划与实际执行之间的偏差，本项目同步维护《SEM-6.6-甘特图.xlsx》的双轨表达方式。每个一级 WBS 使用两行表示：上方蓝色条为计划基线，下方实际执行行用绿色表示提前启动，用橙色表示计划窗口内实际执行，用红色表示实际晚于计划的延期周。图中工作包名称与第 5 节 WBS 表完全一致，时间轴采用第 1 周至第 8 周。
...
| 4.0 | 用户认证与角色权限 | 第 8 周 | 第 7 周至第 8 周 | 0 周 | 权限组件提前接入前端联调，第 8 周完成验收 | 聚焦管理员/工人两个核心角色 |
| 5.0 | 数据查询与统计分析 | 第 1 周至第 6 周 | 第 1 周至第 7 周 | +1 周 | 统计分析、图表和字段联调继续收敛 | 接口字段冻结后集中联调 |
| 6.0 | 业务闭环与时间沙盘 | 第 8 周 | 第 7 周至第 8 周 | 0 周 | 状态机和沙盘规则提前预研，第 8 周集中验收 | 停止非核心扩展，优先保证状态机和沙盘一致 |
| 7.0 | 预算、KPI 与 ROI 决策 | 第 8 周 | 第 7 周至第 8 周 | 0 周 | 预算口径和 ROI 方法学提前联调，第 8 周同步校准 | ROI 方法学和 SEE 经济结论同步校准 |
...
| 10.0 | 持久化与数据库适配 | 第 8 周 | 第 7 周至第 8 周 | 0 周 | 仓储适配层提前预研，第 8 周完成 MySQL 可选增强 | 用仓储适配层隔离文件模式和数据库模式 |
| 11.0 | 测试、验收与结项交付 | 第 7 周至第 8 周 | 第 6 周至第 8 周 | 0 周 | 测试前移，封版周完成验收与提交材料 | 按测试计划、演示脚本和提交清单逐项检查 |

根据修改后的甘特图，关键延期集中在 WBS 3.0 数据资产与知识库，其实际执行比计划多 2 周；WBS 1.0、2.0、5.0、8.0 和 9.0 均表现为 1 周以内的计划外延伸。同时，WBS 4.0、6.0、7.0、10.0 和 11.0 通过提前预研、提前联调或测试前移抵消了第 8 周集中封版压力。它们共同体现了从“可运行主链路”升级为“可解释、可验收、可答辩系统”的返工成本与纠偏过程。团队的纠偏方式是冻结接口字段、停止非核心扩展、把 L0-L3 数据质量和 ROI 经济口径作为封版门禁，并用 WBS 11.0 的测试与验收工作包兜底。

### 6.7 Sprint 燃尽图

燃尽图用于补充说明轻量敏捷过程中的范围消耗和迭代收敛情况。横轴按 S1 至 S6 Sprint 展示，纵轴以 100 个计划价值点表示剩余工作量。理想燃尽线表示按计划均匀完成任务的趋势，实际线表示团队在每轮 Sprint 结束后的剩余工作量。

![Sprint 燃尽图](images/SEM-6.7-sprint-burndown.svg)

从燃尽图可以看出，项目在 S1 至 S4 期间整体按计划下降；S5 因新增角色化工单、时间沙盘、预算 KPI、ROI、可信问答和部署复核等业务闭环增强，实际剩余工作量短暂高于理想燃尽线。S6 阶段团队通过冻结新增范围、集中修复数据口径、补齐权限和部署闭环、统一正式文档事实底稿，使剩余工作快速收敛到提交附件完善层面。

燃尽图与甘特图、挣值曲线的关系如下：

| 图表 | 管理用途 | 本项目解释 |
|---|---|---|
| 甘特图 | 展示 WBS 计划与实际窗口 | 说明哪些工作包延期、延期原因和纠偏措施。 |
| Sprint 燃尽图 | 展示敏捷迭代中的剩余工作消耗 | 说明 S5 范围增加后，S6 如何通过封版门快速收敛。 |
| EV-PV-AC 曲线 | 展示计划价值、挣值和实际消耗 | 说明项目整体进度绩效和返工成本是否可控。 |
...
为展示执行全过程中的资源消耗动态，进一步给出以“累计人时”为单位的挣值曲线。PV 表示计划工作量，EV 表示按完成度折算的挣值工作量，AC 表示实际消耗人时。曲线以《实验三：软件成本估算报告.md》的 1,928.64 人时为计划总量基准，并按第 5 节一级 WBS 的计划窗口进行归一拆分。图中的第一点为项目起点，PV、EV 和 AC 均为 0；随后第 1 周至第 8 周展示各周末累计值。为了避免只用封版结果掩盖过程偏差，周度快照保留了数据资产补齐、前端联调、AI/MCP 补强和封版文档返工带来的折线波动。

| 周次 | PV 累计人时 | EV 累计人时 | AC 累计人时 | SPI | CPI | 过程判断 |
|---|---:|---:|---:|---:|---:|---|
| 第 1 周 | 210.00 | 180.00 | 195.00 | 0.86 | 0.92 | 工程启动，依赖和字段口径磨合导致进度慢于计划 |
| 第 2 周 | 460.00 | 420.00 | 500.00 | 0.91 | 0.84 | 数据、后端和前端并行推进，实际投入上升 |
| 第 3 周 | 710.00 | 650.00 | 745.00 | 0.92 | 0.87 | 数据资产补齐和查询接口联调持续消耗工时 |
| 第 4 周 | 960.00 | 900.00 | 1025.00 | 0.94 | 0.88 | 统计分析和图表口径继续收敛，AC 高于 EV |
| 第 5 周 | 1210.00 | 1120.00 | 1285.00 | 0.93 | 0.87 | L0-L3 数据质量、知识库和经济口径补强形成返工峰值 |
| 第 6 周 | 1460.00 | 1510.00 | 1490.00 | 1.03 | 1.01 | 冻结字段后集中联调，EV 短暂追上 PV 和 AC |
| 第 7 周 | 1710.00 | 1780.00 | 1840.00 | 1.04 | 0.97 | 前端、MCP 和测试继续赶工，进度领先但成本压力回升 |
| 第 8 周 | 1928.64 | 1909.35 | 2012.00 | 0.99 | 0.95 | 封版测试、正式文档和提交附件统一，SPI 接近 1，CPI 低于 1 |

![EV-PV-AC 挣值分析曲线](images/SEM-6.8-EV-PV-AC挣值分析曲线.svg)
...
5. **结项收敛期（第 8 周）**：WBS 1.0 与 WBS 11.0 完成正式文档、验收、测试和提交材料统一。团队通过暂停新增功能、补齐测试、统一事实底稿和更新实验二/三估算，将 EV 拉回 99 点，最终 SPI 维持在 0.99；同时 AC 达到 2,012.00 人时，说明封版返工带来一定成本超支，CPI 约为 0.95。
...
| 环境成本 | 本地开发运行或轻量云服务器演示，默认文件存储，MySQL 可选 | 降低部署门槛，避免依赖 GPU 或高配云资源 |
...
6. 若文档重构成本过高，优先保证 SRS、SDS、SEE、SEM 四份正式文档和 README 一致。
...
6. **文档检查**：提交前统一核对 SRS、SDS、SEE、SEM、README、演示脚本和提交说明。
...
2. **项目管理因素**的核心是范围与文档同步：当系统从看板升级为业务闭环后，规模、成本、测试、SRS、SDS、SEE 和 SEM 必须同步更新。
...
| R2 | 新增需求无法映射到 SRS/SDS 或演示脚本 | 范围评审、阶段门 | 列为后续改进，不进入本期封版 |
...
2. 技术争议先回到 SRS/SDS、API 契约和业务闭环主线，不因临时想法扩大项目范围。
...
|---|---|---|---|
| 模块内实现问题 | 对应模块负责人 | 影响其他模块或阶段门 | SDS 模块设计和测试标准 |
...
| MCP 与最终基础版评审 | 2026-05-31 | 许奕、王天一、马源胜、周由 | REST 主链路和 MCP Server 可作为双接口交付；最终文档需统一项目表述 | 补齐《16-mcp-integration.md》、过程版 SRS/SDS/SEE/SEM 和提交指南 |
...
| 最终封版与答辩材料统筹会 | 2026-06-13 | 许奕、王天一、马源胜、周由 | 正式 SRS、SDS、SEE、SEM、实验二、实验三和演示材料需要统一引用当前封版事实 | 更新正式文档，确认 287 FP、10.71 人月、33.55 万元等规模成本口径 |
...
2. 正式文档用于最终提交和答辩引用。
3. 技术范围发生变化时，SRS、SDS、SEE、SEM 和《30-system-reference-for-final-reports.md》事实底稿应同步更新。
...
| 设计基线 | 正式 SDS 中的架构、模块、数据库、接口和安全设计 | SDS v3.0 完成后 | 实现方案影响验收 |
...
| 测试基线 | 123 个 pytest 用例、MCP 测试和前端构建 | 封版测试通过后 | 新增功能或修复缺陷 |
| 文档基线 | SRS、SDS、SEE、SEM、验收报告、用户手册和演示脚本 | 最终提交前 | 发现关键信息不一致 |

### 11.4 配置审查要求

1. 提交前检查 `.env`、真实 API Key、`data/runtime/`、`frontend/node_modules/`、`frontend/dist/`、`.pytest_cache/` 是否未进入提交包。
2. 检查 README、SRS、SDS、SEE、SEM 和演示脚本中的项目名称、演示账号、访问地址和接口前缀是否一致。
...
| 设计影响 | 是否改变 SDS 中模块、数据库、接口或部署设计 | 标明受影响章节 |
...
| 文档影响 | 是否需要同步 SRS/SDS/SEE/SEM/README | 指定修改责任人 |
...
| 文档验收 | SRS、SDS、SEE、SEM、README、用户手册、演示脚本和提交清单一致性 |
...
| 最终文档 | 许奕 | 全体成员 | SRS、SDS、SEE、SEM、README、提交清单 |
