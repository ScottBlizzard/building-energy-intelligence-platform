...
1. 使用 `admin / admin123` 登录，说明系统面向建筑能源管理与运维支持场景。
2. 在“总览”页展示管理员业务闭环看板、KPI 卡片、三维楼宇风险态势图和数据元信息。
3. 进入“数据浏览”页，演示建筑、楼层、时间和记录数筛选，并演示 CSV 导出。
4. 进入“统计分析”页，展示异常明细并点击“解释”，说明异常触发规则、基线对比、可能原因和处置建议。
5. 继续展示楼层 / 设备台账、趋势、异常原因、设备监测、优化建议、建筑对比和 COP 排名。
6. 进入“工单中心”页，从异常记录中选择一个问题，分配给 `worker_ahu` 或 `worker_chiller`，生成“已派单”工单。
7. 退出管理员，使用对应工人账号登录，在“我的工单”中接单、填写现场原因和处理结果，并提交管理员复核。
8. 切回管理员账号，在“工单中心”查看待复核工单，填写复核意见并关闭工单；工单时间线会保留创建、接单、提交、复核等动作。
9. 进入“决策报告”页，演示未来 7 天能耗预测、节能策略模拟和“一键生成今日运营日报”，说明已关闭工单会进入日报归档口径。
10. 进入“智能问答”页，演示楼层异常、设备优先维护、异常处置建议、节能优化建议和建筑类型差异相关问题。
11. 如需展示 MCP 能力，说明 MCP Server 已把数据查询、统计分析、异常诊断、业务工单看板、运营报告和智能问答封装为可被 AI 客户端调用的 Tools 与 Resources。
