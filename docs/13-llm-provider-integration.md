...
真实 API Key 只能放在本地 `.env` 文件里，不能写入：
...
如果真实 Key 曾经被发到聊天记录、群聊或公开仓库，应立即去对应平台撤销或重置。
...
系统才会尝试调用外部模型。外部模型调用失败时，会自动退回本地规则问答，不影响课堂演示。
...
不要把外部模型作为唯一演示路径。外部 API 可能受余额、限流、网络和平台状态影响。
