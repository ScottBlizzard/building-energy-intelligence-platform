...
本文延续《01-SRS-软件需求规格说明书.md》和《02-SDS-软件设计说明书.md》对系统范围、架构和业务闭环的定义，并按照软件经济分析文档的通用章节结构组织内容。
...
| 《02-SDS-软件设计说明书.md》 | 提供系统架构、模块设计、数据库、接口和安全设计依据 |
...
| 提高项目交付质量 | 用 SRS、SDS、SEE、SEM、测试和演示脚本统一事实，减少答辩和维护返工 |
...
1. 方案 C 与当前代码、正式 SRS/SDS 和最终演示范围一致，验收边界清晰。
...
| 自动化测试文件 | 19 个 | 当前基线为 123 个 pytest 用例全绿 |
| 正式交付文档 | SRS、SDS、SEE、SEM 等 | 配合 README、验收、用户手册、演示脚本和接口说明形成交付包 |
...
| 成本项 | 课程阶段处理方式 |
|---|---|
| 服务器 | 使用本地电脑或轻量云演示服务器，CPU 配置即可运行，不需要 GPU |
...
| 域名与 HTTPS | 课程演示可直接使用服务器公网 IP，正式试点再另行配置域名和 HTTPS |
...
| 范围扩张 | 开发、测试、文档和演示返工成本 | 坚持 SRS/SDS 定义的课程范围 |
...
| 预算健康阈值 | 执行率 `<=100%` | `budget_service.py` |
| 预算预警阈值 | 执行率 `100%-103%` | `budget_service.py` |
| 预算超预算阈值 | 执行率 `>103%` | `budget_service.py` |
| 决策报告预测窗口 | 未来 7 天 | `decision_service.py` |
