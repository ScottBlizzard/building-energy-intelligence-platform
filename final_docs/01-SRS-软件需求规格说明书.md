...
**适用范围**：面向当前封版系统的需求基线，覆盖本地运行、轻量云服务器演示部署、REST API、MCP Server、角色化工单闭环、预算 KPI、ROI 决策和可信智能问答。
**当前基线**：4,864 条样例能耗记录、4 栋建筑、123 个后端 pytest 用例、Vue 前端生产构建和默认 CSV/JSON 离线模式。
...
本文档参考 IEEE 830-1998 的 SRS 编写思路，预期读者包括：
...
本项目为《软件工程管理与经济》课程项目。系统已形成可运行代码，覆盖 FastAPI 后端、Vue 前端、MCP Server、样例能耗数据、知识库、工单闭环、时间沙盘、预算管理、ROI 分析和智能问答等模块。
...
![SRS-2.2-产品功能架构图](images/SRS-2.2-产品功能架构图.png)
...
| 工程支撑与验收支撑 | 启动脚本、测试基线、配置管理和交付验收材料。 |

#### 2.2.1 分层功能拆解

按照课程项目需求文档常用的“层级 + 子模块 + 操作项”树状描述方式，本文档将本系统拆分为数据与知识层、核心业务逻辑层、智能接入层和前端展示层，各层功能如下。
...
| 演示与验收支撑 | 一键重置演示；运行状态提示；关键演示路径支撑。 |

### 2.3 运维闭环流程设计原则

系统核心业务闭环如下图所示。该图用于说明能耗数据、异常诊断、工单执行、维修干预、时间沙盘、预算 ROI、运营报告、AI 助手和 MCP 工具之间的需求关系。

![业务闭环总览图](images/SRS-2.3-business-closure-overview.svg)
...
| 场景编号 | 场景名称 | 主要参与者 | 触发条件 | 期望结果 |
| --- | --- | --- | --- | --- |
| SC-01 | 日常能耗态势查看 | 能源运营管理员 | 管理员进入总览或数据浏览页面。 | 系统展示总电耗、水耗、HVAC 占比、楼层风险、建筑筛选和记录查询结果。 |
| SC-02 | 异常诊断与原因核查 | 能源运营管理员 | 异常列表出现高风险记录。 | 系统给出异常原因、风险分、SLA、浪费电量、费用和碳排估算。 |
| SC-03 | 高风险设备快速派单 | 能源运营管理员、现场工人 | 管理员确认异常需要现场处理。 | 系统检查同设备冲突和工人忙闲，生成工单并进入派单/接单流程。 |
| SC-04 | 现场处理与复核关闭 | 现场工人、能源运营管理员 | 工人完成现场排查并提交复核。 | 管理员复核通过后关闭工单，登记设备级维修干预和完整 timeline。 |
| SC-05 | 时间沙盘验证未来改善 | 能源运营管理员 | 沙盘已启动且存在维修干预。 | 推进业务日期后，系统展示已维修设备未来异常被抑制，不处理设备继续演化。 |
| SC-06 | 预算 KPI 与闭环改善 | 能源运营管理员 | 管理员查看目标建筑年月预算。 | 系统输出健康/预警/超预算状态、五项 KPI 分解评分和闭环节省摘要。 |
| SC-07 | ROI 改造辅助决策 | 能源运营管理员 | 设备反复异常或预算风险较高。 | 系统生成设备审计、NPV、IRR、EAA、回收期和方案比较结论。 |
| SC-08 | 可信问答与 MCP 调用 | AI 助手用户、MCP Client | 用户提出业务问题或智能体调用工具。 | 系统基于真实数据、知识库和工单上下文返回结构化结果和引用。 |
...
本节参考可验收需求写法，将第 4 章用例规约拆分为可检查的原子功能需求。原子需求用于测试、验收和追踪，不替代后续用例中的主成功流程和业务规则。

#### 3.4.1 数据、元信息与总览需求

| 编号 | 优先级 | 需求描述 |
| --- | --- | --- |
| FR-DATA-01 | P0 | 系统应读取 `data/samples/energy_records.csv`，校验必需字段，并在字段缺失时返回明确错误。 |
| FR-DATA-02 | P0 | 系统应提供建筑、楼层、时间范围和 `limit` 筛选能力，Web、导出和 MCP 查询应共享同一过滤口径。 |
| FR-DATA-03 | P0 | 系统应提供数据集元信息，包含记录数、字段列表、字段定义、建筑选项、时间范围、数据字典和字典覆盖率。 |
| FR-DATA-04 | P0 | 系统应按业务日期过滤沙盘可见数据，禁止总览、记录查询、导出、问答和 MCP 出现时间口径不一致。 |
| FR-DATA-05 | P1 | 系统应支持 CSV 导出，并使用当前筛选条件生成导出结果。 |

#### 3.4.2 异常分析与风险量化需求

| 编号 | 优先级 | 需求描述 |
| --- | --- | --- |
| FR-ANALYSIS-01 | P0 | 系统应生成总览 KPI、时间趋势、建筑对比、COP 排名、楼层汇总和设备摘要。 |
| FR-ANALYSIS-02 | P0 | 系统应基于动态基线、COP 阈值、夜间负荷和设备状态识别异常。 |
| FR-ANALYSIS-03 | P0 | 系统应为每条异常输出风险分、严重度、SLA、触发规则和指标证据。 |
| FR-ANALYSIS-04 | P0 | 系统应估算异常浪费电量、费用和碳排，且与预算、报告和问答使用同一常量口径。 |
| FR-ANALYSIS-05 | P1 | 系统应输出优化建议和建议工单，但建议工单不得直接写入持久化状态。 |

#### 3.4.3 用户、权限与工单闭环需求

| 编号 | 优先级 | 需求描述 |
| --- | --- | --- |
| FR-AUTH-01 | P0 | 系统应支持管理员和工人演示账号登录，并返回公开用户信息和 `demo-token:<user_id>`。 |
| FR-AUTH-02 | P0 | 前端应在受保护请求中发送 Bearer token；生产环境后端应拒绝缺失、无效或角色不匹配的 token。 |
| FR-WO-01 | P0 | 管理员应能从异常记录创建工单，系统应保留来源记录、设备、风险、损失和建议动作。 |
| FR-WO-02 | P0 | 同一设备同一时刻只允许存在一张未关闭工单；已登记维修干预的设备不得重复派单。 |
| FR-WO-03 | P0 | 工人只能查看、接单和提交本人被指派的工单，不能创建、派单、复核或关闭工单。 |
| FR-WO-04 | P0 | 工单应支持待确认、已派单、处理中、待复核、已关闭、已驳回和已忽略状态。 |
| FR-WO-05 | P0 | 工单每次创建、派单、接单、提交、复核、关闭、驳回和忽略都应写入 timeline。 |
| FR-WO-06 | P0 | 管理员复核通过后应登记设备级维修干预，并可联动关闭同设备其它未关闭工单。 |

#### 3.4.4 沙盘、预算、ROI 与报告需求

| 编号 | 优先级 | 需求描述 |
| --- | --- | --- |
| FR-SIM-01 | P1 | 系统应支持启动、推进和重置时间沙盘，并以业务日期控制数据可见范围。 |
| FR-SIM-02 | P1 | 系统应在沙盘中支持定时故障和维修干预，使关闭工单能影响未来异常。 |
| FR-SIM-03 | P1 | 系统应提供立即处理、延迟处理和不处理三种反事实方案对比。 |
| FR-BUDGET-01 | P1 | 系统应按建筑和年月生成或维护月度预算，并计算执行率、预测执行率和预算状态。 |
| FR-BUDGET-02 | P1 | 预算状态应按 `<=100%` 健康、`100%-103%` 预警、`>103%` 超预算判定。 |
| FR-BUDGET-03 | P1 | 年度 KPI 应输出预算控制、运行效率、异常风险、工单闭环和改善贡献五项分解评分。 |
| FR-ROI-01 | P1 | ROI 审计应只输出目标建筑真实存在且有实测数据的设备类型。 |
| FR-ROI-02 | P1 | 系统应计算 NPV、IRR、EAA、动态回收期、5 年 ROI、碳价情景和敏感性结果。 |
| FR-REPORT-01 | P1 | 决策报告应汇总风险、工单、预算、ROI、未来 7 天预测、峰值日、趋势和行动建议。 |

#### 3.4.5 智能问答、MCP 与工程支撑需求

| 编号 | 优先级 | 需求描述 |
| --- | --- | --- |
| FR-AI-01 | P1 | AI 助手应支持本地知识库问答、实时业务上下文接地和可选外部 LLM 增强。 |
| FR-AI-02 | P1 | 外部 LLM 调用失败、未配置或事实校验不通过时，系统应回退本地可信回答。 |
| FR-AI-03 | P1 | 问答结果应返回 citations、grounding_sources、grounding_status 和 validation_warnings。 |
| FR-MCP-01 | P1 | MCP Server 应提供数据元信息、记录查询、总览、统计分析、异常解释和运营报告工具。 |
| FR-MCP-02 | P1 | MCP Server 应提供工单上下文、管理员看板、工人看板、知识库检索和问答工具。 |
| FR-MCP-03 | P1 | MCP 问答工具应复用 Web 问答的事实接地和校验逻辑。 |
| FR-ENG-01 | P0 | 系统应提供演示重置能力，恢复可重复演示的工单、预算和沙盘状态。 |
| FR-ENG-02 | P0 | 系统应支持默认 CSV/JSON 离线模式，并在未配置 MySQL、外部 LLM 或真实 `.env` 时可运行。 |
| FR-ENG-03 | P1 | 系统应支持轻量云服务器部署，前端由 Nginx 托管，FastAPI 与 MCP HTTP 服务由 systemd 托管。 |
...
| 用例编号 | 用例名称 | 主要参与者 | 触发条件 | 期望结果 |
| --- | --- | --- | --- | --- |
| UC-00 | 用户登录与权限管理 | 所有用户、系统管理员 | 用户访问系统并输入账号密码。 | 系统返回 token 和角色化工作台，生产环境受保护接口校验 Bearer token。 |
| UC-01 | 管理样例数据与演示状态 | 系统管理员、数据维护者 | 系统启动或执行演示重置。 | 样例数据、数据字典、知识库和运行期状态处于可演示基线。 |
| UC-02 | 查看能源总览与数据查询 | 能源运营管理员、AI 客户端 | 用户选择建筑、楼层或时间范围。 | 系统返回可见数据、总览 KPI、记录表格和导出结果。 |
| UC-03 | 统计分析与异常诊断 | 能源运营管理员、AI 客户端 | 用户查看异常或统计分析页面。 | 系统输出趋势、建筑对比、异常原因、风险分和经济损失。 |
| UC-04 | 派发维修工单 | 能源运营管理员 | 管理员确认异常需要现场处理。 | 系统创建或派发工单，并校验工人忙闲和设备冲突。 |
| UC-05 | 处理现场工单 | 现场工人 | 工人收到本人任务。 | 工人接单、填写处理结果和附件，并提交管理员复核。 |
| UC-06 | 复核关闭工单 | 能源运营管理员 | 工单进入待复核状态。 | 管理员关闭或驳回工单，关闭时登记维修干预。 |
| UC-07 | 时间沙盘与反事实 | 能源运营管理员 | 管理员启动沙盘或选择异常策略。 | 系统推进业务日期，并比较立即、延迟和不处理策略。 |
| UC-08 | 预算执行与闭环改善 | 能源运营管理员 | 管理员查看预算管理页面。 | 系统输出预算状态、KPI 分解评分和闭环改善摘要。 |
| UC-09 | ROI 改造与运营报告 | 能源运营管理员 | 管理员选择建筑和设备改造方案。 | 系统输出 ROI 指标、方案比较和运营报告。 |
| UC-10 | 可信智能问答 | 管理员、现场工人 | 用户提出业务或项目问题。 | 系统返回有引用、接地状态和降级策略的可信回答。 |
| UC-11 | MCP 工具调用 | AI 客户端 | MCP Client 连接并调用工具。 | 系统返回数据、分析、工单、报告和问答工具结果。 |
...
| 次要参与者 | 说明 | 典型关联用例 |
| --- | --- | --- |
| 外部大模型服务（External LLM Service） | 部署在系统边界之外，可选提供语言生成增强；系统调用其接口获取候选回答，但业务事实仍以本系统数据、知识库和校验逻辑为准。 | UC-10 |
...
![UC-00 UML用例图](images/SRS-4.1.1-UC-00登录权限用例图.png)
...
![UC-00 UML泳道活动图](images/SRS-4.1.1-UC-00登录权限泳道活动图.png)

UML 时序图：

![UC-00 UML时序图](images/SRS-4.1.1-UC-00登录权限时序图.png)
...
- BR-103：用户列表接口不得返回密码。
- BR-104：登录成功后，前端应在后续受保护请求中携带 `Authorization: Bearer demo-token:<user_id>`。
- BR-105：生产环境下受保护接口必须以 Bearer token 作为身份依据；开发环境可保留 `operator_id` 作为本地测试兼容参数。
...
![UC-01 UML用例图](images/SRS-4.1.2-UC-01样例数据用例图.png)
...
![UC-01 UML泳道活动图](images/SRS-4.1.2-UC-01样例数据泳道活动图.png)

UML 时序图：

![UC-01 UML时序图](images/SRS-4.1.2-UC-01样例数据时序图.png)
...
- BR-114：数据元信息接口应返回字段列表、建筑选项、时间范围、数据字典、字段定义和字典覆盖率，供 Web 与 MCP 使用同一口径。
...
![UC-02 UML用例图](images/SRS-4.2.1-UC-02数据查询用例图.png)
...
![UC-02 UML泳道活动图](images/SRS-4.2.1-UC-02数据查询泳道活动图.png)

UML 时序图：

![UC-02 UML时序图](images/SRS-4.2.1-UC-02数据查询时序图.png)
...
![UC-03 UML用例图](images/SRS-4.2.2-UC-03异常诊断用例图.png)
...
![UC-03 UML泳道活动图](images/SRS-4.2.2-UC-03异常诊断泳道活动图.png)

UML 时序图：

![UC-03 UML时序图](images/SRS-4.2.2-UC-03异常诊断时序图.png)
...
![UC-04 UML用例图](images/SRS-4.2.3-UC-04派发工单用例图.png)
...
![UC-04 UML泳道活动图](images/SRS-4.2.3-UC-04派发工单泳道活动图.png)
...
![UC-05 UML用例图](images/SRS-4.2.4-UC-05现场处理用例图.png)
...
![UC-05 UML泳道活动图](images/SRS-4.2.4-UC-05现场处理泳道活动图.png)

UML 时序图：

![UC-05 UML时序图](images/SRS-4.2.4-UC-05现场处理时序图.png)
...
![UC-06 UML用例图](images/SRS-4.2.5-UC-06复核关闭用例图.png)
...
![UC-06 UML泳道活动图](images/SRS-4.2.5-UC-06复核关闭泳道活动图.png)
...
![UC-07 UML用例图](images/SRS-4.2.6-UC-07时间沙盘用例图.png)
...
![UC-07 UML泳道活动图](images/SRS-4.2.6-UC-07时间沙盘泳道活动图.png)

UML 时序图：

![UC-07 UML时序图](images/SRS-4.2.6-UC-07时间沙盘时序图.png)
...
![UC-08 UML用例图](images/SRS-4.3.1-UC-08预算闭环用例图.png)
...
![UC-08 UML泳道活动图](images/SRS-4.3.1-UC-08预算闭环泳道活动图.png)

UML 时序图：

![UC-08 UML时序图](images/SRS-4.3.1-UC-08预算闭环时序图.png)
...
- BR-183：预算状态按执行率判定，`<=100%` 为健康，`100%-103%` 为预警，`>103%` 为超预算。
- BR-184：年度 KPI 应输出五项分解评分和原因说明，便于解释扣分或加分来源。
- BR-185：已完整发生的历史月份应使用实际值作为月末口径；仅当前未完成月份才使用预测值估算月末执行率。
...
![UC-09 UML用例图](images/SRS-4.3.2-UC-09ROI报告用例图.png)
...
![UC-09 UML泳道活动图](images/SRS-4.3.2-UC-09ROI报告泳道活动图.png)

UML 时序图：

![UC-09 UML时序图](images/SRS-4.3.2-UC-09ROI报告时序图.png)
...
次要参与者：外部大模型服务（External LLM Service）

UML 用例图：

![UC-10 UML用例图](images/SRS-4.3.3-UC-10智能问答用例图.png)
...
![UC-10 UML泳道活动图](images/SRS-4.3.3-UC-10智能问答泳道活动图.png)
...
![UC-11 UML用例图](images/SRS-4.3.4-UC-11MCP调用用例图.png)
...
![UC-11 UML泳道活动图](images/SRS-4.3.4-UC-11MCP调用泳道活动图.png)

UML 时序图：

![UC-11时序图](images/SRS-4.3.4-UC-11MCP调用时序图.png)
...
| 决策报告 | 管理员 | 生成并展示运营报告、未来 7 天能耗预测、峰值日、趋势和行动建议。 |
...
| IR-REST-02 | 数据 | `GET /overview`、`GET /dataset-meta`、`GET /buildings`、`GET /records` | 总览、字段元信息、数据字典、字典覆盖率、建筑清单和记录查询。 |
...
| 编号 | Tool | 用途 |
| --- | --- | --- |
| IR-MCP-01 | `get_dataset_meta`、`list_buildings`、`query_energy_records` | 数据字段、数据字典、字典覆盖率和记录查询。 |
...
| 编号 | 文件或目录 | 用途 | 约束 |
| --- | --- | --- | --- |
| IR-FILE-01 | `data/samples/energy_records.csv` | 样例能耗数据源。 | 必须包含必需字段；原始数据不得被沙盘或工单流程改写。 |
| IR-FILE-02 | `data/dictionaries/energy_records_dictionary.csv` | 数据字典和字段定义来源。 | `/dataset-meta` 与 MCP `get_dataset_meta` 应读取并暴露覆盖率。 |
| IR-FILE-03 | `data/runtime/work_orders.json` | 默认文件模式下的工单运行期状态。 | 属于运行期数据，不得提交到源码仓库。 |
| IR-FILE-04 | `data/runtime/*budget*.json` | 默认文件模式下的预算运行期状态。 | 演示重置可清理或恢复基线状态。 |
| IR-FILE-05 | `data/runtime/*sim*.json` | 默认文件模式下的时间沙盘状态。 | 沙盘启动、推进、重置和维修干预均应写入同一状态源。 |
| IR-FILE-06 | `knowledge_base/**/*.md` | 本地知识库和问答引用来源。 | 问答和 MCP 检索应返回可追溯引用，不得伪造来源。 |
| IR-FILE-07 | `.env`、`.env.example` | 本地配置和配置模板。 | `.env` 不得提交；`.env.example` 只能包含占位值。 |
| IR-FILE-08 | `frontend/dist` | 生产构建后的前端静态资源。 | 可由 Nginx 托管；源码提交包不要求包含构建产物。 |

### 5.5 外部服务接口

| 编号 | 外部服务或系统 | 用途 | 约束 |
| --- | --- | --- | --- |
| IR-EXT-01 | 可选 MySQL | 替代默认 JSON 运行期存储。 | 未配置 `DATABASE_URL` 时系统必须回退 CSV/JSON 文件模式。 |
| IR-EXT-02 | OpenAI-compatible LLM | 提供可选语言生成增强。 | 失败、超时、未配置或事实校验失败时必须回退本地回答。 |
| IR-EXT-03 | MCP Client / AI 客户端 | 通过 stdio 或 streamable-http 调用项目工具。 | MCP 不得绕过后端服务层和事实校验逻辑。 |
| IR-EXT-04 | Nginx / systemd | 服务器部署时托管前端和进程。 | 轻量云服务器部署仅运行系统本体，不要求部署课程文档或答辩材料。 |
| IR-EXT-05 | 浏览器客户端 | 教师、管理员和工人访问 Web 工作台。 | 应支持现代浏览器；生产演示通过公网 IP 或后续域名访问。 |
...
| NFR-012 | 生产环境下工单、预算、ROI、决策、沙盘写入和演示重置等受保护接口应校验 Bearer token 和管理员角色。 |
| NFR-013 | 工人操作工单时应校验 Bearer token、本人身份和工单归属，拒绝越权访问他人工单。 |
...
| 需求ID | 需求描述 |
| --- | --- |
| NFR-040 | 后端应提供健康检查接口，用于本地开发、服务器部署和验收检查。 |
| NFR-041 | 前端应能展示接口失败、空数据、外部 LLM 降级和未授权操作的明确状态，不应白屏。 |
| NFR-042 | MCP Server 应兼容 stdio 和 streamable-http 两种接入方式。 |
| NFR-043 | 系统应兼容本地开发运行和轻量云服务器部署，不依赖 GPU 或高配算力服务器。 |
| NFR-044 | MySQL、外部 LLM 和公网部署均为可选增强；缺失时不得破坏默认演示链路。 |
| NFR-045 | API 错误响应应保留可理解的错误信息，便于前端、MCP 客户端和验收人员定位问题。 |
...
| 预算状态 | 健康、预警或超预算。 |
| KPI 分数 | 年度考核总分。 |
| KPI 分项评分 | 预算控制、运行效率、异常风险、工单闭环和改善贡献。 |
| KPI 原因说明 | 分项评分对应的解释文本。 |
...
本节保留高层 FR 与用例的追踪关系；第 3.4 节中的原子功能需求可归入对应高层 FR 进行测试和验收。
...
| FR-003 | 角色化工单闭环 | UC-00、UC-04、UC-05、UC-06 | BR-100 至 BR-105、BR-140 至 BR-163 | `auth_service.py`、`api/deps.py`、`permission_service.py`、`work_order_store.py` | P0 |
...
| NFR-010 | 密钥和权限安全 | UC-00 | - | `.gitignore`、`.env.example`、`api/deps.py`、权限测试 | P0 |
...
| 验收编号 | 覆盖需求 | 验收方法 | 验收标准 | 证据材料 |
| --- | --- | --- | --- | --- |
| AC-01 | FR-DATA、FR-001 | 运行数据接口和数据元信息接口。 | 返回 4,864 条样例数据基线、字段定义、数据字典、字典覆盖率、建筑清单和时间范围。 | `/dataset-meta` 响应、数据浏览页面、后端测试。 |
| AC-02 | FR-ANALYSIS、FR-002 | 查看统计分析页面并调用异常解释接口。 | 能展示趋势、建筑对比、COP、异常原因、风险分、浪费费用和碳排。 | 页面截图、API 响应、分析服务测试。 |
| AC-03 | FR-AUTH、FR-WO、FR-003 | 使用管理员和工人账号跑通派单、接单、提交、复核。 | 后端拒绝越权操作；工单状态机、忙闲锁、同设备去重和 timeline 均生效。 | 工单中心演示、权限测试、工单测试。 |
| AC-04 | FR-SIM、FR-004 | 启动沙盘、推进日期并执行反事实分析。 | 查询数据受业务日期约束；关闭工单登记干预；立即/延迟/不处理三策略输出一致。 | 沙盘页面、反事实接口、沙盘测试。 |
| AC-05 | FR-BUDGET、FR-005 | 查看预算管理和 KPI Scorecard。 | 预算状态按健康/预警/超预算判定；KPI 输出五项分解评分和原因说明。 | 预算页面、预算 API、预算测试。 |
| AC-06 | FR-ROI、FR-005 | 选择建筑和设备类型执行 ROI 分析与方案比较。 | 只对真实存在设备生成审计；输出 NPV、IRR、EAA、回收期、敏感性和推荐方案。 | ROI 页面、ROI API、经济口径测试。 |
| AC-07 | FR-REPORT、FR-005 | 打开决策报告页面。 | 报告包含风险、工单、预算、ROI、未来 7 天预测、峰值日、趋势和行动建议。 | 决策报告页面、报告接口响应。 |
| AC-08 | FR-AI、FR-006 | 提问包含工单、异常、预算或项目说明的问题。 | 回答包含引用、接地状态和降级信息；外部 LLM 不可用时仍返回本地可信回答。 | AI 助手页面、问答 API、LLM 降级测试。 |
| AC-09 | FR-MCP、FR-007 | 使用 MCP 测试调用 Tools 和 Resources。 | MCP 能返回数据、分析、工单、报告、知识库和问答结果，且复用 Web 事实校验。 | MCP 测试、MCP 工具列表和响应。 |
| AC-10 | FR-ENG、NFR-010 至 NFR-045 | 运行后端测试、前端构建和健康检查。 | 当前后端 123 个 pytest 用例通过，前端 Vite build 通过，健康检查可访问。 | pytest 输出、build 输出、health 响应。 |
| AC-11 | FR-ENG-03、IR-EXT-04 | 在轻量云服务器访问系统入口。 | Nginx 可访问前端，API 代理到 FastAPI，MCP HTTP 服务可由 systemd 管理。 | 服务器访问地址、systemd 状态、部署配置。 |
| AC-12 | NFR-030 至 NFR-033 | 检查运行期文件和提交包。 | `.env`、真实 Key、`data/runtime/`、`node_modules` 和构建产物不污染源码提交。 | `git status`、`.gitignore`、提交清单。 |

### 8.5 验收结论判定

本项目验收采用“核心需求必须通过、增强需求提供证据、外部依赖可降级”的判定方式：

1. P0/Must 需求对应验收项必须通过，不得只用文字说明替代运行结果。
2. P1/Should 需求应尽量通过页面、接口或测试证明；若外部环境限制，应提供降级响应和配置说明。
3. P2/Could 需求可作为体验增强或后续扩展，不应阻塞核心验收。
4. 外部 LLM、MySQL、MCP HTTP 和公网部署均应说明启用状态；未启用时，默认本地模式仍应可演示。
5. 所有涉及权限、工单状态机、预算 KPI、ROI 和问答事实接地的结论，应能追溯到代码、测试或页面输出。
...
| 阶段五：验收与答辩 | 完成工程支撑材料、演示脚本、最终文档和答辩材料。 | 最终文档、演示路径、测试结果。 |
