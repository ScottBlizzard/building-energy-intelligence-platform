...
- 总记录数 4,864 条，时间范围 2026-01-01 至 2026-06-01
- 每栋建筑各有 1,216 条记录，时间粒度为 3 小时
...
- 不覆盖：节假日独立标记、全年 7～12 月实测数据、分项计量（照明/插座）
- 样例数据已可支撑全部接口联调和课堂演示

**应引用文件：**
- `data/processed/data_quality_report_round1.md`（第 3 节）

**演示适用性：** ✅ 适合，可在项目展示时回答"数据局限性"提问
...
**答案方向：**
- 共 350 条 equipment_status=abnormal 的记录
- 占总记录 7.2%，四栋建筑均有分布
- 异常主要集中在电耗偏高型，无传感器故障型
- 典型案例：R00001（教学楼，凌晨高电耗）、R04606（实验楼，样例期最高电耗 690.04 kWh）
...
- 范围：2.31 ～ 3.24
- 当前告警阈值为 COP < 2.2；本批样例最低 COP 为 2.31，因此没有仅由低 COP 触发的异常
...
**答案方向：**
- 科研实验楼D 电耗最高，3 小时均值 410.5 kWh，最高单次 690.04 kWh
- 其次是图书信息楼C（均值 338.9 kWh）
- 行政办公楼B 最低（均值 235.2 kWh）
...
- 方法二：按“建筑 × 小时时段”建立电耗基线，超过均值 + 2σ 则告警
- 方法三：计算 COP，若低于 2.2 则告警
- 方法四：非工作时段（00:00-06:00）高负荷作为异常原因标签辅助解释
...
- 典型案例：R00024（2026-01-01 21:00），electricity=402.01 kWh，COP=2.34，设备状态为 abnormal
...
- 最高单次电耗 690.04 kWh（R04606），发生在 2026-05-24 15:00，初夏高温下午高峰期
...
**答案方向：**
1. 先确认 COP 是否低于 2.2 阈值；若未低于阈值，也可作为效率偏低的诊断线索
...
**演示适用性：** ✅ 非常适合，结合 R00001 等真实异常记录演示
